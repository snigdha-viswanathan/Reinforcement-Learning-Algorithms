# -*- coding: utf-8 -*-
"""Reinforce Baseline Corrected - MountainCar.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/10G0LJRjq7SGblgS7xni34DIpqQBVfRkk
"""
...
def init_networks(env, hidden_layer_size):
  ...
  #0.009, 0.09
  policy_optim = optim.Adam(policy_network.parameters(), lr=0.01)
  state_value_network = StateValueNetwork(num_observations, hidden_layer_size).to(device)
  state_value_optim = optim.Adam(state_value_network.parameters(), lr=0.1)
...
def reinforce_with_baseline(env, eps, gamma, hidden_layer_size):
  ...
  final_rewards = []
  for episode in tqdm_notebook(range(30)):
    s = env.reset()
    states = []
    actions = []
    rewards = []
...
plt.xlabel('episodes')
plt.title('MountainCar - Reinforce')
plt.show()
...
#Testing
env = gym.wrappers.RecordVideo(env, '/content/drive/MyDrive/RL/', name_prefix="MountainCar")
s = env.reset()
done = False
r = 0
while not done:
